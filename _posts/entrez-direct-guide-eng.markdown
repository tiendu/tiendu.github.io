...
In the example, I have searched the database `bioproject` with the query `txid408169` (representing metagenomes in the field Organism [3]) and Vietnam (in the field Title). After that, I piped the output by using efetch under the format `native`. The raw result will be processed later with awk and sed. Finally, we will have a table with 3 columns: column 1 is the BioProject Title (the name of the study), column 2 is the BioProject Accession, and column 3 is the ID (we're gonna use either the BioProject Accession or the ID to retrieve the reads).

Here, I choose a study in Vietnam about Norovirus with the accession being PRJDB5922 as an example. We also use awk to make it easier to visualize as the original table will be in csv and it's awful to look at in the terminal. Otherwise, you can pipe it into a csv and view it in any editor.

```esearch -db sra -query "PRJDB5922" | efetch -format runinfo | awk 'BEGIN {RS=","; ORS="\t"} {print}'```

We now have some basic information e.g., the sequencing technology used was 454, and it was amplicon sequencing, the source being RNA, etc. We'll redo the step above and pipe it to the other tools to process.
...
I select the first column with `awk 'BEGIN {FS=OFS="\t"} NR > 1 {if ($1 != "") print $1}'`. Then, I use `head -n 3` to get only the first three datasets. In the last command, xargs, I set the number of argument `-n` being 1 and the number of CPUs `-P` being 4. Until then, `fastq-dump` will be used to download the datasets. I download the file in `--gzip`, `--skip-technical`, `--split-files` and `--split-spot`. After the download is finished, we'll have three datasets for practice.
