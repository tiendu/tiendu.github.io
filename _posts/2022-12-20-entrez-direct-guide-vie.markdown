...
Trong hướng dẫn này, chúng ta sẽ sử dụng các tools từ _entrez-direct_ và _sra-tools_ được cung cấp từ NCBI. Yêu cầu tối thiểu gồm có:
...
Thêm vào đó, mình có viết 1 Perl script để lọc các trình tự sử dụng regular expression trên header. Tải về sử dụng ở [4]. Khi sử dụng script, sẽ có 3 parameter mà chúng ta cần nhập: 
- `-i`: input
