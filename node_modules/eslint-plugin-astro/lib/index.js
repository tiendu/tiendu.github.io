...
var eslintCompatUtils = require('eslint-compat-utils');
...
var parser2 = require('astro-eslint-parser');
...
var parser2__namespace = /*#__PURE__*/_interopNamespace(parser2);
...
var init_has_typescript_eslint_parser = __esm({
  "src/configs/has-typescript-eslint-parser.ts"() {
    ...
    try {
      ...
      const relativeTo = path4__default.default.join(cwd, "__placeholder__.js");
      if (tsESLintParser = Module.createRequire(relativeTo)("@typescript-eslint/parser"))
    ...
    overrides: [
      {
        ...
        parser: __require.resolve("astro-eslint-parser"),
...
var baseExtend, recommended_default;
var init_recommended = __esm({
  "src/configs/recommended.ts"() {
    baseExtend = "plugin:astro/base";
    recommended_default = {
      extends: [baseExtend],
      rules: {
        // eslint-plugin-astro rules
...
function getSourceCode(context) {
  return eslintCompatUtils.getSourceCode(context);
}
function getFilename(context) {
  return eslintCompatUtils.getFilename(context);
}
function getCwd(context) {
  return eslintCompatUtils.getCwd(context);
}
...
function isStringCallExpression(node) {
  if (node.type === types.AST_NODE_TYPES.CallExpression) {
    return node.callee.type === types.AST_NODE_TYPES.Identifier && node.callee.name === "String";
  }
  return false;
}
function isStringLiteral(node) {
  return node.type === types.AST_NODE_TYPES.Literal && typeof node.value === "string";
}
function extractConcatExpressions(node, sourceCode) {
  ...
  function processLeft(expr) {
    if (expr.type === types.AST_NODE_TYPES.BinaryExpression) {
      if (!eslintUtils.isParenthesized(expr, sourceCode) && expr.operator !== "*" && expr.operator !== "/") {
...
function getParenthesizedTokens(node, sourceCode) {
  ...
  while ((maybeLeftParen = sourceCode.getTokenBefore(lastLeft)) && (maybeRightParen = sourceCode.getTokenAfter(lastRight)) && eslintUtils.isOpeningParenToken(maybeLeftParen) && eslintUtils.isClosingParenToken(maybeRightParen) && // Avoid false positive such as `if (a) {}`
    ...
    case "NewExpression":
      if (parent.arguments.length === 1 && parent.arguments[0] === node) {
        return sourceCode.getTokenAfter(parent.callee, {
          ...
          filter: eslintUtils.isOpeningParenToken
        });
      }
      return null;
  ...
  "src/rules/no-deprecated-astro-canonicalurl.ts"() {
    ...
    no_deprecated_astro_canonicalurl_default = createRule("no-deprecated-astro-canonicalurl", {
      ...
      create(context) {
        ...
        return {
          "Program:exit"(node) {
            const tracker = new eslintUtils.ReferenceTracker(sourceCode.getScope(node));
            for (const { node: node2, path: path5 } of tracker.iterateGlobalReferences({
              Astro: {
                canonicalURL: { [eslintUtils.READ]: true }
              }
            })) {
              context.report({
                node: node2,
      ...
      create(context) {
        ...
        return {
          "Program:exit"(node) {
            ...
            for (const { node: node2, path: path5 } of tracker.iterateGlobalReferences({
              Astro: {
                fetchContent: { [eslintUtils.READ]: true }
              }
            })) {
              context.report({
                node: node2,
...
function loadModule(context, name2) {
  ...
  try {
    ...
    const relativeTo = path4__default.default.join(cwd, "__placeholder__.js");
    return modules[name2] = Module__default.default.createRequire(relativeTo)(name2);
  } catch {
    return null;
  }
}
...
function transform(node, context) {
  ...
  try {
    ...
    const result = postcss__default.default(config.plugins).process(code, {
      ...config.options,
      map: {
        inline: false
      }
...
function transformToStyleContentCSS(transform5, context) {
  ...
  function remapPosition(pos) {
    decoded = decoded ?? sourcemapCodec.decode(transform5.mappings);
...
function parseSelector(selector, context) {
  let astSelector;
  try {
    astSelector = parser__default.default().astSync(selector);
  } catch (error) {
    return [
      {
        error,
...
function* extractClassListFromExpression(node, context) {
  if (node.type === types.AST_NODE_TYPES.ArrayExpression) {
    for (const element of node.elements) {
      if (element == null) continue;
      if (element.type === types.AST_NODE_TYPES.SpreadElement) {
    ...
    no_unused_css_selector_default = createRule("no-unused-css-selector", {
      ...
      create(context) {
        ...
        function verifyCSS(css) {
          let root;
          try {
            root = postcss__default.default.parse(css.css);
          } catch {
            return;
          }
      ...
      create(context) {
        ...
        return {
          "JSXElement > JSXOpeningElement[name.type='JSXIdentifier'][name.name='style']"(node) {
            ...
            if (!defineVars.value || defineVars.value.type !== types.AST_NODE_TYPES.JSXExpressionContainer || defineVars.value.expression.type !== types.AST_NODE_TYPES.ObjectExpression) {
              return;
            }
            ...
            const definedVars = defineVars.value.expression.properties.filter(
              (prop) => prop.type === types.AST_NODE_TYPES.Property
            ).map((prop) => ({
              prop,
              name: eslintUtils.getPropertyName(prop, sourceCode.getScope(node))
            })).filter(
            ...
            const langValue = lang && lang.value && lang.value.type === types.AST_NODE_TYPES.Literal && lang.value.value;
      ...
      create(context) {
        ...
        class NewObjectProps {
          ...
          fixObject({
            ...
            let text;
            if (eslintUtils.isCommaToken(maybeComma)) {
        ...
        function endsWithSpace(elements) {
          for (let i = elements.length - 1; i >= 0; i--) {
            ...
            if (strings == null) {
              if (valueNode.type === types.AST_NODE_TYPES.TemplateLiteral) {
        ...
        function verifyAttr(attr) {
          ...
          if (!attr.value || attr.value.type !== types.AST_NODE_TYPES.JSXExpressionContainer || attr.value.expression.type === types.AST_NODE_TYPES.JSXEmptyExpression) {
            return;
          }
                ...
                if (element.array) {
                  ...
                  if (object) {
                    ...
                    if (eslintUtils.isCommaToken(maybeComma)) {
                      ...
                      if (eslintUtils.isCommaToken(maybeBeforeComma)) {
          ...
          function findClosestObject(array, target) {
            ...
            for (let index2 = 0; index2 < length; index2++) {
              ...
              if (after?.type === types.AST_NODE_TYPES.ObjectExpression) {
                return after;
              }
              const before = beforeElements[index2];
              if (before?.type === types.AST_NODE_TYPES.ObjectExpression) {
                return before;
              }
            }
            return null;
          ...
          function visitElementExpression(node, state) {
            if (node.type === types.AST_NODE_TYPES.ConditionalExpression) {
              verifyConditionalExpression(node, state);
            } else if (node.type === types.AST_NODE_TYPES.TemplateLiteral) {
              const quasis = [...node.quasis];
            ...
            } else if (node.type === types.AST_NODE_TYPES.CallExpression) {
              if (isStringCallExpression(node) && node.arguments[0] && node.arguments[0].type !== types.AST_NODE_TYPES.SpreadElement) {
              ...
              } else if (node.callee.type === types.AST_NODE_TYPES.MemberExpression && eslintUtils.getPropertyName(node.callee) === "trim") {
            ...
            } else if (node.type === types.AST_NODE_TYPES.BinaryExpression) {
      ...
      create(context) {
        ...
        function verifyAttr(attr) {
          ...
          verifyExpression(expression, function* (fixer) {
            if (expression.type === types.AST_NODE_TYPES.ArrayExpression) {
              return;
            }
        ...
        function verifyExpression(node, transformArray, call) {
          if (node.type === types.AST_NODE_TYPES.TemplateLiteral) {
          ...
          } else if (node.type === types.AST_NODE_TYPES.ArrayExpression) {
            for (const element of node.elements) {
              if (element) {
                verifyExpression(element, transformArray);
              }
            }
          } else if (node.type === types.AST_NODE_TYPES.Literal) {
            ...
            if (node.callee.type === types.AST_NODE_TYPES.MemberExpression && eslintUtils.getPropertyName(node.callee) === "trim") {
        ...
        function verifyClsx(clsxCall) {
          if (clsxCall.node.type !== types.AST_NODE_TYPES.CallExpression) {
            return;
          }
          const callNode = clsxCall.node;
          const parent = callNode.parent;
          if (!parent || parent.type !== types.AST_NODE_TYPES.JSXExpressionContainer || parent.expression !== callNode) {
            return;
          }
          const parentParent = parent.parent;
          if (!parentParent || parentParent.type !== types.AST_NODE_TYPES.JSXAttribute || parentParent.value !== parent || getAttributeName(parentParent) !== "class:list") {
            return;
          }
          context.report({
        ...
        return {
          Program(node) {
            ...
            const referenceTracker = new eslintUtils.ReferenceTracker(sourceCode2.getScope(node));
    ...
    semi_default = createRule("semi", {
      ...
      create(context) {
        ...
        function isRedundantSemi(semiToken) {
          ...
          return !nextToken || eslintUtils.isClosingBraceToken(nextToken) || eslintUtils.isSemicolonToken(nextToken);
        }
        function isEndOfArrowBlock(lastToken) {
          if (!eslintUtils.isClosingBraceToken(lastToken)) return false;
        ...
        function checkForSemicolon(node) {
          const lastToken = sourceCode.getLastToken(node);
          const isSemi = eslintUtils.isSemicolonToken(lastToken);
          if (never) {
            const nextToken = sourceCode.getTokenAfter(node);
...
function requireUserLocal(id) {
  try {
    const cwd = process.cwd();
    const relativeTo = path4__default.default.join(cwd, "__placeholder__.js");
    return Module.createRequire(relativeTo)(id);
  } catch {
    return null;
  }
}
...
function isLinterPath(p) {
  return p.includes(`eslint${path4__default.default.sep}lib${path4__default.default.sep}linter${path4__default.default.sep}linter.js`);
}
function getEspree() {
  if (!espreeCache) {
...
${code}
  ...
  parser2.traverseNodes(expression, {
...
`);
      ...
      extractDefineVars() {
        ...
        if (expression.type !== types.AST_NODE_TYPES.ObjectExpression) return [];
        const startLoc = this.parsed.getLocFromIndex(valueStart + 1);
        return expression.properties.filter((p) => p.type === types.AST_NODE_TYPES.Property).filter((p) => !p.computed).map((p) => {
          return {
            name: p.key.type === types.AST_NODE_TYPES.Identifier ? p.key.name : p.key.value,
            loc: {
...
function preprocess(code, filename, virtualFileExt) {
  if (filename) {
    ...
    let parsed;
    try {
      parsed = parser2.parseTemplate(code);
    } catch {
      return [code];
    }
...
function buildLegacyConfigs() {
  const baseExtend2 = "plugin:astro/base";
  const configs = {};
  for (const configName of a11yConfigKeys) {
    Object.defineProperty(configs, `jsx-a11y-${configName}`, {
      enumerable: true,
...
var init_all = __esm({
  "src/configs/all.ts"() {
    ...
    all_default = {
      extends: recommended_default.extends,
      rules: {
        ...all,
        ...recommended_default.rules
      }
    };
  }
});
...
var init_recommended2 = __esm({
  "src/configs/flat/recommended.ts"() {
    init_base2();
    recommended_default2 = [
      ...base_default,
      {
...
var init_all2 = __esm({
  "src/configs/flat/all.ts"() {
    ...
    for (const rule of rules.filter(
      ...
      all2[rule.meta.docs.ruleId] = "error";
    }
    all_default2 = [
      ...recommended_default2,
      {
        rules: {
          ...all2,
          ...recommended_default2[recommended_default2.length - 1].rules
        }
      }
    ];
  }
...
var init_cjs_config_builder = __esm({
  "src/cjs-config-builder.ts"() {
    init_base();
    init_recommended();
    init_all();
...
var require_src = __commonJS({
  "src/index.cts"(exports, module) {
    init_cjs_config_builder();
    init_plugin();
    var configs = buildCjsConfigs();
    module.exports = Object.assign(plugin2, { configs });
  }
});
