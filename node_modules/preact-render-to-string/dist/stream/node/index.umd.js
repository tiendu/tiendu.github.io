(function (global, factory) {
	...
	const HTML_ENUMERATED = new Set(['draggable', 'spellcheck']);
	const COMPONENT_DIRTY_BIT = 1 << 3;
	...
	const END_SUSPENSE_DENOMINATOR = '<!--/$s-->'; // Global state for the current render pass
	...
	function renderClassComponent(vnode, context) {
	  ...
	  c[VNODE] = vnode;
	  c.props = vnode.props;
	  c.context = context; // Turn off stateful rendering
	  ...
	  if (typeof type == 'function') {
	    ...
	    if (type === preact.Fragment) {
	      ...
	      if (isClassComponent && preact.options.errorBoundaries && (type.getDerivedStateFromError || component.componentDidCatch)) {
	        ...
	        try {
	          return _renderToString(rendered, context, isSvgMode, selectValue, vnode, asyncMode, false, renderer);
	        } catch (err) {
	    ...
	    try {
	      ...
	      if (vnode._suspended) {
	        ...
	        return str.then(resolved => BEGIN_SUSPENSE_DENOMINATOR + resolved + END_SUSPENSE_DENOMINATOR);
	      }
	      ...
	      const renderNestedChildren = () => {
	        try {
	          const result = _renderToString(rendered, context, isSvgMode, selectValue, vnode, asyncMode, renderer);

	          return vnode._suspended ? BEGIN_SUSPENSE_DENOMINATOR + result + END_SUSPENSE_DENOMINATOR : result;
	        } catch (e) {
